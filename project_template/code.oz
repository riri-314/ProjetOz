local
   ...
   fun {PartitionToTimedList L T E} %L=List (Partiton) T=Time of the note (1.0 by default, T=1.0 means that the note will last 1.0 sec)  E=stretch factor (1.0 by default, ex: if T=1.5 and E=2.0 then the new T=3.0)
      case L
      of H1|T1 then
          case H1
          of H2|T2 then 
              ({NoteToExtended H2 T}|{PartitionToTimedList T2 T E})|{PartitionToTimedList T1 T E} %{NoteToExtended H2 T} 
          [] duration(1:P seconds:R) then 
              %{Browse R/{IntToFloat {Lenght P}}}
